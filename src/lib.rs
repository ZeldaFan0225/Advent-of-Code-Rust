            ...
            for day in days {
                match day {
                    $(_ if day == $crate::day_to_number!($day) => {
                        ...
                        if input.is_empty() {
                            error!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Input file is empty. Skipping!");
                            continue;
                        } else if std::str::from_utf8(input).is_err() {
                            error!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Input file must be valid UTF-8. Skipping!");
                            continue;
                        }/* else if input[input.len() - 1] != b'\n' {
                            error!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Input file must end with a trailing newline. Skipping!");
                            continue;
                        }*/
                        ...
                        $(
                            ...
                            if result == $p1_exp {
                                info!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Part 1: {result} == {}", $p1_exp);
                                debug!(target: "advent_of_code_rust", "{}", easybench::bench(|| $day::part_1(input)));
                            } else {
                                warn!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Part 1: {result} != {}", $p1_exp);
                            }
                        )?
                        ...
                        $(
                            ...
                            if result == $p2_exp {
                                info!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Part 2: {result} == {}", $p2_exp);
                                debug!(target: "advent_of_code_rust", "{}", easybench::bench(|| $day::part_2(input)));
                            } else {
                                warn!(target: "advent_of_code_rust", "[Year {year}] [Day {day}] Part 2: {result} != {}", $p2_exp);
                            }
                        )?
                    })*
